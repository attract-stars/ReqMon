...
public class CreateHelper {
	...
	public CreateHelper(Display display, Shell shell, Boolean isApp, RequirementElement reqElem,
			SystemElement sysElem) {
	...
	public void createTop(Composite top) {
		...
		reqButton.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				...
				// table.createTable();
//TODO INTEGRATE THE File DIALOG
...
//					}
	...
	public void createBoxItem(Composite parent, int index, int num) {
		...
		try {
			...
			new ContentProposalAdapter(text, new TextContentAdapter(),
					new SimpleContentProposalProvider(new String[] { "PorposalOne", "ProposalTwo", "ProposalThree" }),
	...
	private void updateList() {
		...
		for (Control con : compList.getChildren()) {
			con.dispose();
		}
		for (int i = 0; i < reqElem.getElementSize(); i++) {
			createBoxItem(compList, i, i + 1);
			compList.pack();
			compList.layout(true);
			// shell.pack();
			// shell.layout(true);
			updateMinSize(compList.getParent());
		}
	}
